...
import { cn } from '@/lib';
import { backupPlugins } from '@/features/tools/websiteCalculator/utils/data/Backup.const';
...
export function WebCalcBackupStep() {
  // Get the backup plugin from the store
  const { backup, updatePlugin } = usePlugins();
  ...
  const {
    ...
    getPluginPrice,
    formData,
  } = usePluginStep('backup', backupPlugins, backup.name, backup.version);
  ...
  useEffect(() => {
    if (selectedPlugin && selectedVersion) {
      ...
      if (
        backup.name !== selectedPlugin ||
        backup.version !== selectedVersion ||
        backup.price !== price
      ) {
        updatePlugin('backup', {
          name: selectedPlugin,
            ...
            <div className="p-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {plugin.free ? (
                  ...
                  <div className="rounded-lg border-2 border-foreground/20 p-4 bg-background-primary/20">
