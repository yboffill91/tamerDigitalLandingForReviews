...
export function WebCalcAntispamStep() {
  // Get the antispam plugin from the store
  const { antispam, updatePlugin } = usePlugins();
    ...
    <PluginStepLayout
      ...
      priceSummary={
        formData.purpose === 'client' && (
          <PluginPriceSummary
            title="Total Anti-spam Setup Cost"
      ...
      footer={
        <div className="flex justify-center items-center gap-6 w-full mt-12">
          ...
          <ShimmerButton
            ...
            className={`${antispam.name ? '' : 'hidden'}`}
          >
            Next
          </ShimmerButton>
        </div>
