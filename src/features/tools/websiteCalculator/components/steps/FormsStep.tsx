      ...
      {formData.purpose === 'client' && (
        <div className="bg-gradient-to-br from-primary/10 to-ring/20 border border-ring rounded-lg p-6 mb-6">
          <h3 className="text-xl font-semibold text-primary mb-2">
            Total Forms Setup Cost
          </h3>
          <p className="text-3xl font-bold">${getPluginPrice().toFixed(2)}</p>
          <p className="text-sm text-primary mt-2">
            Includes professional setup, configuration, and form creation
          </p>
        </div>
      )}

      <div className="grid gap-6">
        {formsPlugins.map(plugin => (
          <div
            key={plugin.name}
            className={cn(
                  ...
                  <div className="rounded-lg border-2 border-foreground/20 p-4 bg-ring/20">
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center">
                        <span className="inline-block px-3 py-1 bg-primary text-blue-200 rounded-full text-sm font-medium mb-2">
                          Premium Only Plugin
                        </span>
