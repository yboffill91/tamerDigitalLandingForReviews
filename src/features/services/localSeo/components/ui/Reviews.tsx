...
export const FeaturedImages = ({
  ...
  return (
    <div
      className={`flex flex-col items-center lg:items-start mt-10 mb-10 ${
        containerClassName || ''
      }`}
    >
      <div
        className={`flex flex-col lg:flex-row items-center gap-4 lg:gap-6 mb-2 ${
          className || ''
        }`}
      >
        {/* Images container */}
        ...
        {showStars && (
          <div className='flex items-center justify-center lg:self-center mt-4 lg:mt-0'>
            {[...Array(5)].map((_, index) => (
              <BsStarFill
                key={index}
                className='h-4 w-4 text-yellow-400 fill-yellow-500'
              />
            ))}
          </div>
        )}
      ...
      <p
        className={`text-white/80 text-sm text-center lg:text-left ml-0 lg:ml-8 relative z-40 ${
          textClassName || ''
        }`}
      >
